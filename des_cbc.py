...
# функция Фейстеля
# f применяется к функции R-1(32bits) и K1(48bits)
def f(R,K):
    R48=[i for i in range(48)]
    for i in range(48):
        R48[i]=R[expansion_table[i]]
    after_XOR=[R48[j]^K[j] for j in range(48)]

    #S-boxes 8 S-boxes размера 6bits как входные данные
    after_sbox=[]
    for j in range(8):
...
def CBC_DES_ENC(IV,text,key_hexa):
     # IV : это 64-битный бинарный вектор и текст: строка символов, а key_hexa строка HEX символов 
    ...
    if(last_block_size != 0):
        ...
        cipher_block = DES(XOR_res, key_binary)
        cypher_text += cipher_block
    cypher_text_ascii_string=[chr(int("".join([str(cypher_text[i + j]) for j in range(8)]),2)) for i in range(0,len(cypher_text)-1,8)]
...
def CBC_DES_DEC(IV,cyphertext,key_hexa): 
    ...
    text_ascii_string = [chr(int("".join([str(text[i + j]) for j in range(8)]), 2)) for i in range(0, len(text)-1, 8)]
...
key = input("Введите ключ (только HEX символы /([0-9]|[a-f])/):")

cypher_text=CBC_DES_ENC(IV,text, key)
original_text=CBC_DES_DEC(IV,cypher_text,key)

print(f"Шифротекст: {cypher_text}")
